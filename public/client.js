...
var readSyncStep2 = (decoder, doc3, transactionOrigin, errorHandler) => {
  try {
...
var readSyncMessage = (decoder, encoder, doc3, transactionOrigin, errorHandler) => {
  ...
  switch (messageType) {
    ...
    case messageYjsSyncStep2:
      readSyncStep2(decoder, doc3, transactionOrigin, errorHandler);
      break;
    case messageYjsUpdate:
      readUpdate(decoder, doc3, transactionOrigin, errorHandler);
      break;
    default:
...
// node_modules/.pnpm/y-partyserver@1.0.0_@cloudflare+workers-types@4.20260217.0_partyserver@0.1.5_@cloudflar_7a4cb90513b1811cdc8a28a7f0e1d49d/node_modules/y-partyserver/dist/provider/index.js
var messageSync = 0;
...
messageHandlers[messageSync] = (encoder, decoder, provider2, emitSynced, _messageType) => {
  ...
  if (emitSynced && syncMessageType === messageYjsSyncStep2 && !provider2.synced) provider2.synced = true;
};
...
messageHandlers[messageAuth] = (_encoder, decoder, provider2, _emitSynced, _messageType) => {
  readAuthMessage(decoder, provider2.doc, (_ydoc, reason) => permissionDeniedHandler(provider2, reason));
};
var messageReconnectTimeout = 3e4;
function permissionDeniedHandler(provider2, reason) {
  console.warn(`Permission denied to access ${provider2.url}.
${reason}`);
}
function readMessage(provider2, buf, emitSynced) {
  const decoder = createDecoder(buf);
  const encoder = createEncoder();
  const messageType = readVarUint(decoder);
  const messageHandler = provider2.messageHandlers[messageType];
  if (messageHandler) messageHandler(encoder, decoder, provider2, emitSynced, messageType);
  else console.error("Unable to compute message");
  return encoder;
}
function setupWS(provider2) {
  if (provider2.shouldConnect && provider2.ws === null) {
    if (!provider2._WS) throw new Error("No WebSocket implementation available, did you forget to pass options.WebSocketPolyfill?");
    ...
    websocket.addEventListener("message", (event) => {
      if (typeof event.data === "string") {
        if (event.data.startsWith("__YPS:")) {
          ...
          provider2.emit("custom-message", [customMessage]);
        }
        return;
      }
    ...
    websocket.addEventListener("close", (event) => {
      ...
      if (provider2.wsconnected) {
        ...
        provider2.emit("status", [{ status: "disconnected" }]);
      } else provider2.wsUnsuccessfulReconnects++;
    ...
    websocket.addEventListener("open", () => {
      ...
      provider2.emit("status", [{ status: "connected" }]);
      const encoder = createEncoder();
      writeVarUint(encoder, messageSync);
    ...
    provider2.emit("status", [{ status: "connecting" }]);
  }
}
function broadcastMessage(provider2, buf) {
  const ws = provider2.ws;
  if (provider2.wsconnected && ws && ws.readyState === ws.OPEN) ws.send(buf);
  if (provider2.bcconnected) publish(provider2.bcChannel, buf, provider2);
}
var DefaultWebSocket = typeof WebSocket === "undefined" ? null : WebSocket;
var WebsocketProvider = class extends Observable {
  constructor(serverUrl, roomname, doc3, { connect = true, awareness = new Awareness(doc3), params: params2 = {}, isPrefixedUrl = false, WebSocketPolyfill = DefaultWebSocket, resyncInterval = -1, maxBackoffTime = 2500, disableBc = DEFAULT_DISABLE_BC } = {}) {
    super();
    __publicField(this, "maxBackoffTime");
    __publicField(this, "bcChannel");
    __publicField(this, "url");
    __publicField(this, "roomname");
    __publicField(this, "doc");
    __publicField(this, "_WS");
    __publicField(this, "awareness");
    __publicField(this, "wsconnected");
    __publicField(this, "wsconnecting");
    __publicField(this, "bcconnected");
    __publicField(this, "disableBc");
    __publicField(this, "wsUnsuccessfulReconnects");
    __publicField(this, "messageHandlers");
    __publicField(this, "_synced");
    __publicField(this, "ws");
    __publicField(this, "wsLastMessageReceived");
    __publicField(this, "shouldConnect");
    __publicField(this, "_resyncInterval");
    __publicField(this, "_bcSubscriber");
    __publicField(this, "_updateHandler");
    __publicField(this, "_awarenessUpdateHandler");
    __publicField(this, "_unloadHandler");
    ...
    this._checkInterval = setInterval(() => {
      if (this.wsconnected && messageReconnectTimeout < getUnixTime() - this.wsLastMessageReceived) {
        assert2(this.ws !== null, "ws must not be null");
        this.ws.close();
      }
...
var _params, _a;
var YProvider = (_a = class extends WebsocketProvider {
  constructor(host, room, doc3, options = {}) {
    ...
    const id2 = options.connectionId ?? nanoid(10);
    ...
    super(serverUrl, room, doc3 ?? new Doc(), baseOptions);
  ...
  async connect() {
    try {
      const nextParams = typeof __privateGet(this, _params) === "function" ? await __privateGet(this, _params).call(this) : __privateGet(this, _params);
...
}, _params = new WeakMap(), _a);

// src/client.js
var doc2 = new Doc();
var provider = new YProvider(location.host, DIGEST_ID, doc2, {
  party: "digest-object"
});
var config = doc2.getMap("config");
var topicsArr = doc2.getArray("topics");
var digestsArr = doc2.getArray("digests");
...
function renderConfig() {
  ...
  const input = $2("#email-input");
  if (document.activeElement !== input && email) {
    input.value = email;
  }
  $2("#enabled-toggle").checked = enabled;
}
function renderTopics() {
  const topics = topicsArr.toArray();
  const list = $2("#topic-list");
  list.innerHTML = "";
...
function renderDigests() {
  ...
  const container = $2("#digest-history");
  if (!digests.length) {
    container.innerHTML = '<p class="empty-state">No digests yet. Add topics and generate your first one!</p>';
    return;
  }
  container.innerHTML = [...digests].reverse().map(
    (d, i) => `
  ...
  container.querySelectorAll(".digest-entry-header").forEach((el) => {
    el.addEventListener("click", () => {
      document.getElementById(`digest-body-${el.dataset.idx}`).classList.toggle("open");
    });
  });
}
...
function showStatus(selector, msg, ok) {
  ...
  if (ok) setTimeout(() => el.textContent = "", 3e3);
}
$2("#save-email").addEventListener("click", () => {
  const email = $2("#email-input").value.trim();
  if (!email) return;
  doc2.transact(() => {
...
$2("#add-topic").addEventListener("click", addTopic);
$2("#topic-input").addEventListener("keydown", (e) => {
  if (e.key === "Enter") addTopic();
});
$2("#trigger-btn").addEventListener("click", () => {
  const btn = $2("#trigger-btn");
  btn.disabled = true;
...
$2("#enabled-toggle").addEventListener("change", (e) => {
